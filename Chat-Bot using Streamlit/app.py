...
# Tải dữ liệu trực tiếp từ CSV thay vì Google Sheet
@st.cache_data
def load_csv_data():
    try:
        # Đọc dữ liệu từ string CSV
...
"""
        ...
        if empty_line_indices:
            ...
            # Nếu không tìm thấy dòng trống, xử lý toàn bộ dữ liệu như một bảng
...
# CSS tùy chỉnh để có giao diện đẹp hơn
st.markdown("""
<style>
    .main-header {
...
# Thử lấy dữ liệu từ CSV
csv_data = load_csv_data()

# Thanh bên với thông tin
with st.sidebar:
    ...
    # Khám phá dữ liệu CSV nếu có sẵn
    if csv_data:
...
# Định nghĩa các biến thể lời chào
greetings = [
...
# Định nghĩa các biến thể lời tạm biệt
farewells = [
...
# Cơ sở kiến thức với các phản hồi toàn diện hơn
knowledge_base = {
    "schedule": [
...
# Hàm để nhận các phản hồi nhận biết ngữ cảnh từ dữ liệu CSV
def get_bot_response(message, csv_data=None):
    message = message.lower()
    
    # Hàm trợ giúp để tìm thông tin từ dữ liệu CSV theo từ khóa
    def find_csv_info(keyword, csv_data):
        ...
        # Tìm kiếm trong lịch hoạt động
        if "Lịch hoạt động" in csv_data:
            schedule_df = csv_data["Lịch hoạt động"]
            
            # Tìm trong cột Thời gian và Hoạt động
            for index, row in schedule_df.iterrows():
                ...
                if keyword in activity or keyword in time_slot:
                    ...
                    # Xác định nhóm tuổi từ câu hỏi
                    selected_age = "Nhóm 3 – 4 tuổi"  # Mặc định
        ...
        if "Thực đơn" in csv_data:
            ...
            # Xác định loại bữa ăn từ câu hỏi
            selected_meal = None
    ...
    # Kiểm tra xem người dùng có đang hỏi về dữ liệu CSV không
    csv_keywords = {
    ...
    # Xác định câu trả lời dựa trên các từ khóa trong câu hỏi
...
with st.form(key="chat_form", clear_on_submit=True):
    ...
    if submit_button and user_input:
        ...
        # Cập nhật trang để hiển thị tin nhắn mới
        st.rerun()
